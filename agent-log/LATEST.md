# LATEST.md — pointer file (not a symlink, plain copy)

**This file is a manually-updated pointer, not a symlink.** Symlinks
don't survive every file transfer/zip tool reliably, so this system
uses a plain copy instead: `LATEST.md` always contains an exact copy
of the current latest session file's content.

**When you create a new session file, you must ALSO overwrite this
file with the same content** (or just copy the new file over this
one). See agent-log/INDEX.md for the exact step.

---

<!-- CONTENT BELOW IS AN EXACT COPY OF THE LATEST SESSION FILE -->
